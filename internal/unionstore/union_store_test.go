...
func TestUnionStoreIterReverseWithBound(t *testing.T) {
	assert := assert.New(t)
	store := newMemDB()
	us := NewUnionStore(&mockSnapshot{store})
	...
	iter, err := us.IterReverse(nil, []byte("1")) // bound included.
	assert.Nil(err)
	checkIterator(t, iter, [][]byte{[]byte("3"), []byte("2"), []byte("1")}, [][]byte{[]byte("3"), []byte("2"), []byte("1")})
	...
	iter, err = us.IterReverse([]byte("3"), []byte("2"))
	assert.Nil(err)
	...
	iter, err = us.IterReverse([]byte("3"), []byte("0"))
	assert.Nil(err)
	checkIterator(t, iter, [][]byte{[]byte("2"), []byte("1"), []byte("0")}, [][]byte{[]byte("2"), []byte("1"), []byte("0")})
